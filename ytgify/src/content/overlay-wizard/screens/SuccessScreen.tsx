...
const SuccessScreen: React.FC<SuccessScreenProps> = ({
  ...
  const [showSignupCta, setShowSignupCta] = React.useState(true);
  ...
  // Check if user has permanently dismissed the signup CTA
  React.useEffect(() => {
    const checkDismissed = async () => {
      try {
        const result = await chrome.storage.local.get('signupCtaDismissed');
        if (result.signupCtaDismissed) {
          setShowSignupCta(false);
        }
      } catch (error) {
        console.error('Error checking signup CTA dismissal:', error);
      }
    };
    checkDismissed();
  }, []);

  // Handle permanent dismissal of signup CTA
  const handleDismissSignupCta = async () => {
    setShowSignupCta(false);
    try {
      await chrome.storage.local.set({ signupCtaDismissed: true });
    } catch (error) {
      console.error('Error saving signup CTA dismissal:', error);
    }
  };
  ...
  const handleSignUp = () => {
    ...
    openExternalLink(`${signupUrl}/users/sign_up?source=extension-wizard`);
  };
  ...
  return (
    <div className="ytgif-wizard-screen ytgif-success-screen">
      ...
      <div className="ytgif-wizard-content">
        ...
        {/* Sign Up CTA for unauthenticated users (permanently dismissible) */}
        {!isAuthenticated &&
          showSignupCta &&
          uploadStatus !== 'uploading' &&
          uploadStatus !== 'success' && (
            <div
              className="ytgif-signup-cta"
              style={{
                position: 'relative',
                textAlign: 'center',
                marginTop: '16px',
            ...
            >
              {/* Dismiss button */}
              <button
                onClick={handleDismissSignupCta}
                style={{
                  position: 'absolute',
                  top: '4px',
                  right: '4px',
      ...
      {showFooter && (
        <div className="ytgif-wizard-footer">
          ...
          <button className="dismiss-btn" onClick={handleDismissFooter}>
            ×
          </button>
        </div>
      )}
